...
double forwardCoeff(const double s, const int r)
{
    double temp = 1;
    for (int i = 0; i < r; i++)
    {
...
double forwardPhi(const unsigned numberOfEntries, const double w, double const *x, double const *y)
{
    double s, h, temp = 0;
    h = x[1] - x[0];
    s = ((w - x[0]) / h);
    ...
    {
        temp += forwardCoeff(s, r) * forwardDel(r, 0, y);
    }
    return temp;
}
void printTable(const unsigned numberOfEntries, const double *x, const double *y)
{
    ...
    for (unsigned i = 0; i < numberOfEntries; i++)
    {
        ...
        {
            if (j > numberOfEntries - i - 1)
                continue;
            printf("%lf\t", forwardDel(j, i, y));
        }
        printf("\n");
    }
}
void main()
{
    ...
    scanf("%lf", &w);
    printf("f(%.2lf) = %.2lf", w, forwardPhi(numberOfEntries, w, x, y));
}